    ...
    public static class TBSUtil
    {
        ...
        // where Tile : class
        {
            ...
            while (queue.Count > 0)
            {
                ...
                Tile current = pair.Value.FirstOrDefault(tile => !tile.Equals(from));
        ...
        ////A* Pathfinding Algorithm
        //public static List<Tile> PathFind<Tile>(MTRandom random, Tile from, Tile to,
        //        Func<Tile, IEnumerable<Tuple<Tile, double>>> GetNeighbors, Func<Tile, Tile, double> GetDistance)
        //        // where Tile : class
        //{
        //    if (random == null)
        //        throw new ArgumentNullException("random");
        ...
        //    double dist = GetDistance(to, from);

        //    //priority queue of tiles to be traversed
        //    var queue = new SortedDictionary<double, HashSet<Tile>> { { dist, new HashSet<Tile> { to } } };
        //    //tiles already traversed
        //    var closed = new HashSet<Tile>();

        //    //distance from start along best known path
        //    var distTo = new Dictionary<Tile, double> { { to, 0 } };
        //    //best guess total distance when moving through tile; back-index doubleo queue
        //    var distThrough = new Dictionary<Tile, double> { { to, dist } };
        ...
        //        double curDist = distTo[current];
        //        foreach (var tuple in GetNeighbors(current))
        //        {
        //            Tile neighbor = tuple.Item1;
        //            double newDist = curDist + tuple.Item2;
        ...
        //            //default to MaxValue so that the inner if will evaluate true
        //            double priorDist = double.MaxValue;
        ...
        //                if (newDist < priorDist)
        //                {
        //                    double newGuess = newDist + GetDistance(neighbor, from);
        ...
        //}
        //A* Pathfinding Algorithm
        public static List<Tile> PathFind<Tile>(MTRandom random, Tile from, Tile to, Func<Tile, bool> Stop,
                Func<Tile, IEnumerable<Tuple<Tile, double>>> GetNeighbors, Func<Tile, Tile, double> GetDistance)
        // where Tile : class
        {
            if (random == null)
            ...
            //priority queue of tiles to be traversed
            var queue = new SortedDictionary<double, HashSet<Tile>> { { dist, new HashSet<Tile> { from } } };
            //tiles already traversed
            var closed = new HashSet<Tile>();

            //distance from start along best known path
            var distTo = new Dictionary<Tile, double> { { from, 0 } };
            //best guess total distance when moving through tile; back-index into queue
            var distThrough = new Dictionary<Tile, double> { { from, dist } };

            var solutions = new Dictionary<Tile, List<Tile>>();

            while (queue.Count > 0)
            {
                //select the next tile from the queue
                var pair = queue.First();
                if (!pair.Value.Any())
                    continue;
                Tile current = random.SelectValue(pair.Value);

                //solution found
                if (current.Equals(to) || Stop(current))
                    return Traverse(random, solutions, current, from);

                double curDist = distTo[current];
                foreach (var tuple in random.Iterate(GetNeighbors(current)))
                {
                    Tile neighbor = tuple.Item1;
                    ...
                    if ((inQueue == null && !closed.Contains(neighbor)) || newDist <= (priorDist = distTo[neighbor]))
                        ...
                        if (newDist < priorDist)
                        {
                            double newGuess = newDist + GetDistance(neighbor, to);
        ...
        private static void Dequeue<Tile>(SortedDictionary<double, HashSet<Tile>> queue, double key, HashSet<Tile> set, Tile tile)
        {
            if (set.Count == 1)
                queue.Remove(key);
            else
        ...
        private static List<Tile> Traverse<Tile>(MTRandom random, Dictionary<Tile, List<Tile>> solutions, Tile from, Tile to) // where Tile : class
        {
            Tile current = from;
            List<Tile> path = new() { current };
            while (!current.Equals(to))
            {
                current = random.SelectValue(solutions[current]);
                path.Add(current);
            }
            path.Reverse();
            return path;
