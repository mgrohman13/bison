...
namespace Gravity
{
    class Game : MattUtil.RealTimeGame.Game
    {
        ...
        public const float offMapPull = .005f;
        ...
        public Game(double gameTick, MattUtil.RealTimeGame.GameTicker.EventDelegate RefreshGame, Rectangle rectangle) : base(gameTick, RefreshGame)
        {
            ...
            player = new Player(this, rand.Gaussian(playerDist), rand.Gaussian(playerDist), 25f, 1);
            target = new Target(this, rand.Gaussian(playerDist), rand.Gaussian(playerDist), 15f, 1);

            pieces = new HashSet<Piece>() { new Center(this, 0, 0, 5f, 5f), player, target };
        ...
        public override void Step()
        {
            ...
            if (rand.Bool(.025 * Math.Pow(avgNum / ( avgNum + this.pieces.OfType<Enemy>().Count() ), 5)))
                NewEnemy();
        }
        ...
        {
            Console.WriteLine("remove Enemy");
            pieces.Remove(enemy);
        }
