...
namespace GalWarWin
{
    public partial class ShipDesignSortForm : Form
    {
        ...
        internal static double GetValue(ShipDesign shipDesign)
        {
            ...
            {
                int att = form.cbAttack.Checked ? shipDesign.Att : 1;
                int def = form.cbDefense.Checked ? shipDesign.Def : 1;
                ...
                double bombardDamage = ds ? shipDesign.BombardDamage / att / Consts.BombardAttackMult : 0;
                if (!form.cbSpeed.Checked)
                {
                    trans *= Game.Random.Round(shipDesign.Speed / 2.0);
                    bombardDamage *= shipDesign.Speed / 2.0;
                }
                if (!form.cbAttack.Checked)
                {
                    bombardDamage *= shipDesign.Att;
                }
